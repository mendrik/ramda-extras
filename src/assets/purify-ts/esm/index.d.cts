...
declare const enumeration: <T extends Record<string, string | number>>(e: T) => Codec<T[keyof T]>;
...
declare const oneOf: <T extends [Codec<any>, ...Codec<any>[]]>(codecs: T) => Codec<GetType<T extends (infer U)[] ? U : never>>;
...
declare const record: <K extends string | number | symbol, V>(keyCodec: Codec<K>, valueCodec: Codec<V>) => Codec<Record<K, V>>;
/** A codec that only succeeds decoding when the value is exactly what you've constructed the codec with */
declare const exactly: <T extends (string | number | boolean)[]>(...expectedValues: T) => Codec<T[number]>;
...
declare const tuple: <TS extends [Codec<any>, ...Codec<any>[]]>(codecs: TS) => Codec<{ [i in keyof TS]: TS[i] extends Codec<infer U> ? U : never; }>;
...
declare const map: <K, V>(keyCodec: Codec<K>, valueCodec: Codec<V>) => Codec<Map<K, V>>;
type ExpectedType = 'string' | 'number' | 'boolean' | 'object' | 'array' | 'null' | 'undefined' | 'enum';
type ReceivedType = 'string' | 'number' | 'boolean' | 'object' | 'array' | 'null' | 'undefined' | 'bigint' | 'symbol' | 'function';
type DecodeError = {
    type: 'property';
    property: string;
    error: DecodeError;
...
type CurriedFn<TAllArgs extends any[], TReturn> = <TProvidedArgs extends TAllArgs extends [infer TFirstArg, ...infer TRestOfArgs] ? [TFirstArg, ...Partial<TRestOfArgs>] : never>(...args: TProvidedArgs) => TProvidedArgs extends TAllArgs ? TReturn : TAllArgs extends [...TupleOfLength<TProvidedArgs>, ...infer TRestOfArgs] ? CurriedFn<TRestOfArgs, TReturn> : never;
...
declare const curry: <TArgs extends any[], TReturn>(fn: (...args: TArgs) => TReturn) => CurriedFn<TArgs, TReturn>;
...
declare function sort<T>(compare: (a: T, b: T) => Order, list: readonly T[]): T[];
declare function sort<T>(compare: (a: T, b: T) => Order): (list: readonly T[]) => T[];
declare const List: {
...
export { Codec, CurriedFn, DecodeError, Either, EitherAsync, EitherAsyncTypeRef, EitherPatterns, ExpectedType, FromType, GetType, just as Just, left as Left, List, Maybe, MaybeAsync, MaybeAsyncTypeRef, MaybePatterns, NonEmptyList, NonEmptyListCompliant, NonEmptyListTypeRef, nothing as Nothing, Order, ReceivedType, right as Right, Tuple, TupleTypeRef, always, array, boolean, compare, curry, date, enumeration, exactly, identity, intersect, lazy, map, maybe, nonEmptyList, nullType, nullable, number, oneOf, optional, orderToNumber, parseError, record, string, tuple, unknown };
